...
import { SlangData } from '../components/SlangCard';
...
export function EditSlang() {
  ...
  useEffect(() => {
    const fetchSlang = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          const data = docSnap.data() as SlangData;
          setWord(data.word);
          setPronunciation(data.pronunciation || '');
          setMeaning(data.meaning);
          setMeaningBurmese(data.meaningBurmese || '');
          setExamples(data.examples && data.examples.length > 0 ? data.examples : ['']);
  ...
  if (!user || (appUser?.role !== 'moderator' && appUser?.role !== 'admin')) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">Only moderators and admins can edit slangs.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 font-medium rounded-xl transition-all w-fit shadow-sm hover:shadow mb-8"
      >
      ...
      <div className="mb-8 text-center">
        ...
        <h1 className="text-4xl font-display font-bold text-slate-900 tracking-tight">Edit Slang</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-10 rounded-3xl border border-slate-200 shadow-sm space-y-8">
        <div className="space-y-6">
          ...
          <div>
            <label htmlFor="status" className="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wider">
              Status
            </label>
            <select
              id="status"
              className="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white outline-none transition-all text-base font-medium appearance-none"
              value={status}
